...
{  
    double power,value;  
    int i,sign;  
    assert(s!=NULL);//ÅÐ¶Ï×Ö·ûŽ®ÊÇ·ñÎª¿Õ  
    for(i=0;is_space(s[i]);i++);//³ýÈ¥×Ö·ûŽ®Ç°µÄ¿Õžñ  
    sign=(s[i]=='-')?-1:1;  
    if(s[i]=='-'||s[i]=='+')//ÒªÊÇÓÐ·ûºÅÎ»ŸÍÇ°œøÒ»Î»  
        i++;  
    for(value=0.0;is_digit(s[i]);i++)//ŒÆËãÐ¡ÊýµãÇ®µÄÊý×Ö  
        value=value*10.0+(s[i]-'0');  
    if(s[i]=='.')  
        i++;  
    for(power=1.0;is_digit(s[i]);i++)//ŒÆËãÐ¡ÊýµãºóµÄÊý×Ö  
    {  
        value=value*10.0+(s[i]-'0');  
        power*=10.0;  
    }  
...
/*-----------------------------------------------------------------------------
  ...
  ·µ»ØÖµ:      ÔÚ¹æ¶šµÄÊ±ŒäÄÚ¶ÁÈ¡ÊýŸÝ£¬³¬Ê±ÔòÍË³ö£¬³¬Ê±Ê±ŒäÎªmsŒ¶±ð
  ÃèÊö:        ÏòfdÃèÊö·ûµÄŽ®¿ÚœÓÊÕÊýŸÝ£¬³€¶ÈÎªlen£¬ŽæÈëstr£¬timeout Îª³¬Ê±Ê±Œä
 *-----------------------------------------------------------------------------*/
int serial_read(int fd, unsigned char *str, unsigned int len,unsigned char *cmp_data,int cmp_len)
{
        ...
        int ret;		//Ã¿ŽÎ¶ÁµÄœá¹û
        int sret;		//selectŒà¿Øœá¹û
        int readlen = 0;	//ÊµŒÊ¶ÁµœµÄ×ÖœÚÊý
        unsigned char * ptr;
...
#ifdef ARMLINUX
//check_f ÊÇ·ñÐèÒªÐ£Ñé  Ö÷ÒªÐèÒªžÄÕâžöº¯Êý
int receive_send(int fd,unsigned char *Send_str,int Send_len,unsigned char *cmp_data,int Receive_len,int cmp_len,int check_f)
  {
    ...
    if(Send_len!=0)
         ...
         //    write(fd,Send_str,Send_len); //·¢ËÍ×Ö·û³öÈ¥
          ...
          //   write(fd,&send_checksum,1);  //·¢ËÍÐ£ÑéÂë
          }
...
int transmit(int fd, unsigned char *buffer,int length, int length2)
  {
    ...
    for(i=0;i<length-4;i++)
      {
        send_data1[5+i]=*buffer;  // ·¢ËÍµÄÊýŸÝsend_data1
        buffer++;
      }
    ...
    while(1)
       {
         ...
         if(i==1)
           {
             for(i=0;i<length2;i++)
             result[i]=receive_data[5+i]; //receive_data Ç°5Î»ÊÇÓŠŽðÐÅºÅ
                          ...
                          if(memcmp(p1,"a(",2)==0)
                                {
                                 ...
                                 for(i=0;*p1!=NULL;i++)
                                   ...
                                   if(memcmp(p1,"step",4)==0)
                                     {
                                        p2[25]=(unsigned char)y;
                                        p2[24]=(unsigned char)(y>>8);
                                        p2[26]=0x00;
                                        p2[27]=0x00;
                                     }
                                   else
                                   ...
                                   if(memcmp(p1,"step",4)==0)
                                     {
                                        ...
                                        frec(x,2,data_cmp1);
                                        //p2[34]=p2[10]|(data_cmp1[0]>>2);//07-7-13 18:17œâÊÍÏàÎ»²œ³€ŽíÎó£¬µŒÖÂÏàÎ»×Ô¶¯ÔÚ±ä»¯
    ...
    else if(memcmp(p1,"inp:",4)==0)
            {
             ...
             if(memcmp(p1,"ana:",4)==0)
            ...
            else if(memcmp(p1,"dig:interval(",13)==0)  //¿ª¹ØÁ¿·Ö±æÂÊ¡£
                 {
                   ...
                   inp_data[9]=(unsigned char)(time_out); //ÔÚÏµÍ³ÉèÖÃÀïÔöŒÓ¡°¿ª¹ØÁ¿·Ö±æÂÊ¡±ºó
        ...
        if(channel_status==1)
          {
            ...
            if (phan!=360)
               phan+=VaPhaseRevision;//07-10-08 16:10,ÏàÎ»Ð£×Œ
          }
        else if(channel_status==2)
          {
            ...
            if (phan!=360)  //2008-12-25
               phan+=VbPhaseRevision;//07-10-08 16:10,ÏàÎ»Ð£×Œ
          }
        else if(channel_status==3)
          {
            ...
            if(phan!=360)  //2008-12-25
               phan+=VcPhaseRevision;//07-10-08 16:10,ÏàÎ»Ð£×Œ
          }
        ...
        if(channel_status==1)
          {
            ...
            if(phan!=360)  //2008-12-25
               phan+=IaPhaseRevision;//07-10-08 16:10,ÏàÎ»Ð£×Œ
          }
        else if(channel_status==2)
          {
            for(i=0;i<40;i++)
            ...
            if(phan!=360)  //2008-12-25
               phan+=IbPhaseRevision;//07-10-08 16:10,ÏàÎ»Ð£×Œ
          }
        else if(channel_status==3)
          {
            for(i=0;i<40;i++)
            ...
            if(phan!=360)  //2008-12-25
               phan+=IcPhaseRevision;//07-10-08 16:10,ÏàÎ»Ð£×Œ
          }
      }
    if(phan<0)
      phan+=360;
...
int read_max(int fd)
 {
  ...
  printf("%s %d Vabcz %f  %f  %f %f\n",__func__,__LINE__,max_output_v_a,max_output_v_b,max_output_v_c,max_output_v_z);
   printf("%s %d Iabc %f  %f  %f\n",__func__,__LINE__,max_output_i_a,max_output_i_b,max_output_i_c);
   ...
   artExec(fd,"others:revision:get",result,59);//¶ÁÆµÂÊ¡¢ÏàÎ»²¹³¥²ÎÊý
   response[0]=result[7];
   response[1]=result[8];
   response[2]=result[9];
   FreqRevision=BytesToFloat(response,4,3,SIGNED);//ÆµÂÊ²¹³¥²ÎÊý

   response[0]=result[10];
   response[1]=result[11];
   response[2]=result[12];
   VaPhaseRevision=BytesToFloat(response,4,3,SIGNED);//VaÏàÎ»²¹³¥²ÎÊý
   response[0]=result[13];
   response[1]=result[14];
   response[2]=result[15];

   VbPhaseRevision=BytesToFloat(response,4,3,SIGNED);//VbÏàÎ»²¹³¥²ÎÊý
   response[0]=result[16];
   response[1]=result[17];
   response[2]=result[18];
   VcPhaseRevision=BytesToFloat(response,4,3,SIGNED);//VcÏàÎ»²¹³¥²ÎÊý


   response[0]=result[22];
   response[1]=result[23];
   response[2]=result[24];
   IaPhaseRevision=BytesToFloat(response,4,3,SIGNED);//IaÏàÎ»²¹³¥²ÎÊý
   response[0]=result[25];
   response[1]=result[26];
   response[2]=result[27];
   IbPhaseRevision=BytesToFloat(response,4,3,SIGNED);//IbÏàÎ»²¹³¥²ÎÊý
   response[0]=result[28];
   response[1]=result[29];
   response[2]=result[30];
   IcPhaseRevision=BytesToFloat(response,4,3,SIGNED);//IcÏàÎ»²¹³¥²ÎÊý

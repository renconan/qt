...
float thread1::CheckAngle(QVector<ACTSTATE>  vect,int type)
{
  ...
  /*
  else if(type==2) //找第一个边界角度
  {
      int size=vect.size() ;
      if(vect.at(0).Triped!=vect.at(size-1).Triped)  //前提是有第一个边界角度
          return -2; //未找到第二个边界
      return vect.at(size-1).Angle;  //找到第二个边界S
  }
  ...
  artExec(temprorarydata->UartDev[0].uartfd,"sys:cfg?",result,31);
   ...
   while(Threadrunstate)
    ...
    if(temprorarydata->keypadopenflag==false&&temprorarydata->helpopenflag==false)
     ...
     if(starttest==0)
 {
     emit SIG_TestState();//2016-5-17测试完毕，text不切换问题修改

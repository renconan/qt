...
//        artExec(fd,m_Ch[4].getOnSign(false),result,0);//IA放大器对应DB2
//    else
//       artExec(fd,m_Ch[4].getOnSign(true),result,0);
    artExec(fd,m_Ch[4].getOnSign(fabs(m_Ch[4].OutputValue)>0.00001),result,0);//IA放大器对应DB2
    ...
    if(k==0)
    ...
    else if(k==1)
      {
        switch(cAnytest.Variable)
         ...
         {
            //故障前参�
            A_V1PerFaultValue=cAnytest.SysVolt;//57.73;
            A_V2PerFaultValue=cAnytest.SysVolt;//57.73;
            A_V3PerFaultValue=cAnytest.SysVolt;//57.73;
            A_VzPerFaultValue=cAnytest.SysVolt;//57.73;
            A_I1PerFaultValue=0;
            A_I2PerFaultValue=0;
            A_I3PerFaultValue=0;

            A_V1PerFaultPhase=m_Ch[0].Phase-360*cAnytest.SysFrec*cAnytest.PreFaultTime;
            A_V2PerFaultPhase=m_Ch[1].Phase-360*cAnytest.SysFrec*cAnytest.PreFaultTime;
            A_V3PerFaultPhase=m_Ch[2].Phase-360*cAnytest.SysFrec*cAnytest.PreFaultTime;
            A_VzPerFaultPhase=m_Ch[3].Phase-360*cAnytest.SysFrec*cAnytest.PreFaultTime;
            ...
            A_I1PerFaultPhase=m_Ch[4].Phase-360*cAnytest.SysFrec*cAnytest.PreFaultTime;
            A_I2PerFaultPhase=m_Ch[5].Phase-360*cAnytest.SysFrec*cAnytest.PreFaultTime;
            A_I3PerFaultPhase=m_Ch[6].Phase-360*cAnytest.SysFrec*cAnytest.PreFaultTime;
            ...
            channel_input(fd,1,A_V1PerFaultValue,A_V1PerFaultPhase,cAnytest.SysFrec/FreqTriple,m_Ch[0].DC,1,0,0,0);
            artExec(fd,"out:ana:v(1:1):on",result,0);
            channel_input(fd,1,A_V2PerFaultValue,A_V2PerFaultPhase,cAnytest.SysFrec/FreqTriple,m_Ch[1].DC,2,0,0,0);
            artExec(fd,"out:ana:v(1:2):on",result,0);
            channel_input(fd,1,A_V3PerFaultValue,A_V3PerFaultPhase,cAnytest.SysFrec/FreqTriple,m_Ch[2].DC,3,0,0,0);
            artExec(fd,"out:ana:v(1:3):on",result,0);

            channel_input(fd,1,A_VzPerFaultValue,A_VzPerFaultPhase,cAnytest.SysFrec/FreqTriple,m_Ch[3].DC,4,0,0,0);
            artExec(fd,"out:ana:v(1:4):on",result,0);

            channel_input(fd,0,A_I1PerFaultValue,A_I1PerFaultPhase,cAnytest.SysFrec/FreqTriple,m_Ch[4].DC,1,0,0,0);
            artExec(fd,"out:ana:i(1:1):on",result,0);
            channel_input(fd,0,A_I2PerFaultValue,A_I2PerFaultPhase,cAnytest.SysFrec/FreqTriple,m_Ch[5].DC,2,0,0,0);
            artExec(fd,"out:ana:i(1:2):on",result,0);
            channel_input(fd,0,A_I3PerFaultValue,A_I3PerFaultPhase,cAnytest.SysFrec/FreqTriple,m_Ch[6].DC,3,0,0,0);
            artExec(fd,"out:ana:i(1:3):on",result,0);

            artExec(fd,"inp:buf:sam(bin,1)",result,0);

            if(checkiszero(cAnytest.PreFaultTime)==false)
            out_time=cAnytest.PreFaultTime; //;  故障前时�
            else
            out_time=-1;
            ...
            out_time=cAnytest.FaultTime; //故障时间
            ...
            // 故障�
             ...
             out_time=cAnytest.PostFaultTime; // 故障后时�第三�
             ...
             out_time=cAnytest.PostFaultTime;     //故障后时�第四�
             artExec(fd,"seq:wait(0,1)",result,0);
...
void Output_signal_Auto(int fd)
  {
    ...
    if(fabs(m_Ch[0].Amp)>0.00001 || (m_Ch[0].checked==1 && cAnytest.Variable==1))  //2016-6-4自动测试�开始未报警修改
      m_Ch[0].Output=1;
    else
      m_Ch[0].Output=0;

    if(fabs(m_Ch[1].Amp)>0.00001 || (m_Ch[1].checked==1 && cAnytest.Variable==1))
      m_Ch[1].Output=1;
    else
      m_Ch[1].Output=0;

    if(fabs(m_Ch[2].Amp)>0.00001 || (m_Ch[2].checked==1 && cAnytest.Variable==1))
      m_Ch[2].Output=1;
    else
      m_Ch[2].Output=0;

    if(fabs(m_Ch[3].Amp)>0.00001 || (m_Ch[3].checked==1 && cAnytest.Variable==1))
          m_Ch[3].Output=1;
        else
          m_Ch[3].Output=0;

    if(fabs(m_Ch[4].Amp)>0.00001 || (m_Ch[4].checked==1 && cAnytest.Variable==1))  //2016-6-4自动测试�开始未报警修改
      m_Ch[4].Output=1;
    else
      m_Ch[4].Output=0;

    if(fabs(m_Ch[5].Amp)>0.00001 || (m_Ch[5].checked==1 && cAnytest.Variable==1))
      m_Ch[5].Output=1;
    else
      m_Ch[5].Output=0;

    if(fabs(m_Ch[6].Amp)>0.00001 || (m_Ch[6].checked==1 && cAnytest.Variable==1))
      m_Ch[6].Output=1;
    else
      m_Ch[6].Output=0;

  change_timedata=cAnytest.StepTime;
    ...
    switch(cAnytest.Variable)
      {
          case 1:
              if(m_Ch[0].checked==1)
                VStep1=cAnytest.VolStep;
              if(m_Ch[1].checked==1)
                VStep2=cAnytest.VolStep;
              if(m_Ch[2].checked==1)
                VStep3=cAnytest.VolStep;
              if(m_Ch[3].checked==1)
                VStep4=cAnytest.VolStep;


              if(m_Ch[4].checked==1)
                CStep1=cAnytest.CurStep;
              if(m_Ch[5].checked==1)
                CStep2=cAnytest.CurStep;
              if(m_Ch[6].checked==1)
                CStep3=cAnytest.CurStep;
              V1RelaPhase=0;
              V2RelaPhase=0;
              V3RelaPhase=0;
              VzRelaPhase=0;
          ...
          case 2:
              if(m_Ch[0].checked==1)
                VPStep1=cAnytest.PhaStep;
              if(m_Ch[1].checked==1)
                VPStep2=cAnytest.PhaStep;
              if(m_Ch[2].checked==1)
                VPStep3=cAnytest.PhaStep;
              if(m_Ch[3].checked==1)
                VPStep4=cAnytest.PhaStep;


              if(m_Ch[4].checked==1)
                PStep1=cAnytest.PhaStep;
              if(m_Ch[5].checked==1)
                PStep2=cAnytest.PhaStep;
              if(m_Ch[6].checked==1)
                PStep3=cAnytest.PhaStep;
              //幅度步长
              VStep1=0;
              VStep2=0;
              VStep3=0;
          ...
          case 3:
              if(m_Ch[0].checked==1)
                VFStep1=cAnytest.FreStep;
              if(m_Ch[1].checked==1)
                VFStep2=cAnytest.FreStep;
              if(m_Ch[2].checked==1)
                VFStep3=cAnytest.FreStep;

              if(m_Ch[3].checked==1)
                VFStep4=cAnytest.FreStep;


              if(m_Ch[4].checked==1)
                FStep1=cAnytest.FreStep;
              if(m_Ch[5].checked==1)
                FStep2=cAnytest.FreStep;
              if(m_Ch[6].checked==1)
                FStep3=cAnytest.FreStep;
    ...
    artExec(fd,"inp:buf:sam(bin,2)",result,0);
    out_time=fabs((cAnytest.StepNum+1)*cAnytest.StepTime);

...
extern int binOut;
Manualtest::Manualtest(QString ModelID,CWindowsBase *parent) :
    CWindowsBase(ModelID,parent),
...
//    ui->label_26->setPixmap(img);
    ...
    display_mode(cAnytest.TestMode);
    ...
    display_variry(cAnytest.Variable);
...
//    connect(uartthread,SIGNAL(SIG_TestButtenState()),SLOT(GetTestButtenstate()));
...
int Manualtest::StartTestState()
{
    if(starttest==1)return 0;
    on_pushButton_6_clicked();
    return 1;
}
int Manualtest::StopTestState()
{
    if(starttest==0)return 0;
    on_pushButton_6_clicked();
    return 1;
}
void Manualtest::UpDatePara(QMap<QString,float>* mp)
{//perror("update-start");
    ...
    while(iter!=mp->end())
    {
        if(iter.key()=="VaV")
        {
            m_Ch[0].Amp=iter.value();
        }
        else if(iter.key()=="VaP")
        {
            m_Ch[0].Phase=iter.value();
        }
        else if(iter.key()=="VaF")
        {
            m_Ch[0].Freq=iter.value();
        }
        else if(iter.key()=="IaV")
        {
            m_Ch[4].Amp=iter.value();
        }
        else if(iter.key()=="IaP")
        {
            m_Ch[4].Phase=iter.value();
        }
        else if(iter.key()=="IaF")
        {
            m_Ch[4].Freq=iter.value();
        }
        else if(iter.key()=="VaC")
        {
            m_Ch[0].checked=iter.value();
        }
        else if(iter.key()=="IaC")
        {
            m_Ch[4].checked=iter.value();
        }

        else if(iter.key()=="Mod")
        {
            cAnytest.TestMode=iter.value();
            display_mode(cAnytest.TestMode);
        }
        else if(iter.key()=="Var")
        {
            cAnytest.Variable=iter.value();
            display_variry(cAnytest.Variable);
        }

        else if(iter.key()=="PfT")
        {
            cAnytest.PreFaultTime=iter.value();
        }
        else if(iter.key()=="fT")
        {
            cAnytest.FaultTime=iter.value();
        }
        else if(iter.key()=="EfT")
        {
            cAnytest.PostFaultTime=iter.value();
        }


        else if(iter.key()=="IS")
        {
            cAnytest.CurStep=iter.value();
        }
        else if(iter.key()=="VS")
        {
            cAnytest.VolStep=iter.value();
        }
        else if(iter.key()=="PS")
        {
            cAnytest.PhaStep=iter.value();
        }
        else if(iter.key()=="FS")
        {
            cAnytest.FreStep=iter.value();
        }
        else if(iter.key()=="TS")
        {
            cAnytest.StepTime=iter.value();
        }
        else if(iter.key()=="NS")
        {
            cAnytest.StepNum=iter.value();
        }


        else if(iter.key()=="HT")
        {
            faultduration=iter.value();
        }
        iter++;
    }
    initalldata();
...
void Manualtest::init_par()
{
    ...
    cAnytest.SysFrec=SysFrec2*2;
    cAnytest.SysVolt=mSysVolt;
    LogicResolution=mLogicResolution;
    cAnytest.PreFaultTime=mPreFaultTime;
    cAnytest.PostFaultTime=mPostFaultTime;
    Preparetime=mPreparetime;

    fclose(fp);
    //exit_par=0;
    m_Ch[0].Amp=cAnytest.SysVolt;//50;
    m_Ch[0].Phase=0;
    m_Ch[0].Freq=cAnytest.SysFrec;
    cAnytest.Variable=1;
    cAnytest.TestMode=1;
    ...
    m_Ch[1].Amp=cAnytest.SysVolt;//57.73;
    m_Ch[1].Phase=-120;
    m_Ch[1].Freq=cAnytest.SysFrec;

    m_Ch[2].Amp=cAnytest.SysVolt;
    m_Ch[2].Phase=120;
    m_Ch[2].Freq=cAnytest.SysFrec;

    m_Ch[3].Amp=cAnytest.SysVolt;
    m_Ch[3].Phase=0;
    m_Ch[3].Freq=cAnytest.SysFrec;
    ...
    VariatyVz2=0;
       ...
       m_Ch[4].Amp=3;
       m_Ch[4].Phase=0;
       m_Ch[4].Freq=cAnytest.SysFrec;

       m_Ch[5].Amp=0;
       m_Ch[5].Phase=-120;
       m_Ch[5].Freq=cAnytest.SysFrec;

       m_Ch[6].Amp=0;
       m_Ch[6].Phase=120;
       m_Ch[6].Freq=cAnytest.SysFrec;


       cAnytest.CurStep=0.5;
       cAnytest.VolStep=1;
       cAnytest.PhaStep=1;
       cAnytest.FreStep=1;

       cAnytest.FaultTime=5;

       cAnytest.StepTime=0.5;
       cAnytest.StepNum=20;
       if(cAnytest.SysFrec==0)
       {
       m_Ch[0].DC=1;
       m_Ch[1].DC=1;
       m_Ch[2].DC=1;
...
void Manualtest::initalldata()
{
  ...
  ui->lineEdit_19->setText(QString::number(cAnytest.CurStep,'g',4));
  ui->lineEdit_20->setText(QString::number(cAnytest.VolStep,'g',4));
  ui->lineEdit_21->setText(QString::number(cAnytest.PhaStep,'g',4));
  ui->lineEdit_22->setText(QString::number(cAnytest.FreStep,'g',4));
  ui->lineEdit_23->setText(QString::number(cAnytest.StepTime,'g',4));
  ui->lineEdit_24->setText(QString::number(cAnytest.StepNum,'g',4));

  ui->lineEdit_25->setText(QString::number(cAnytest.PreFaultTime,'g',4));
  ui->lineEdit_26->setText(QString::number(cAnytest.FaultTime,'g',4));
  ui->lineEdit_27->setText(QString::number(cAnytest.PostFaultTime,'g',4));
...
void Manualtest::showdspstate(int state)
{
    ...
    case 1: ui->label_10->setText(tr("电流温度或功率保护")); break;
    case 2: ui->label_10->setText(tr("电流过载或接线不全")); break;
    case 3: ui->label_10->setText(tr("电流闭锁")); break;
    case 4: ui->label_10->setText(tr("电压温度保护")); break;
    case 5: ui->label_10->setText(tr("电压过载")); break;
    }
...
float Manualtest::showPhase(float value)//显示结果，相位值的处理
{
    ...
    ftemp=value+A_SumStepnum*cAnytest.PhaStep;
    if(ftemp>360)
    {
        temp=(int)(ftemp/360);
    ...
    QString testStr = "";
        ...
        if(cAnytest.TestMode==2)
       {
          ...
          else
         {
          ...
          testStr = "TripTime="+QString::number(TripTime,'g',6);
          emit SIG_SendTestResult(testStr);
       }
        ...
        else if(cAnytest.TestMode==3)
        {
            ...
            else
          {
            ...
            case 1:  //幅值
                if(m_Ch[0].checked==1)
                {
                 ui->textBrowser->append(tr("VA:")+QString::number(m_Ch[0].Amp+A_SumStepnum*cAnytest.VolStep,'g',6)+tr(" V"));
                }
                if(m_Ch[1].checked==1)
                {
                 ui->textBrowser->append(tr("VB:")+QString::number(m_Ch[1].Amp+A_SumStepnum*cAnytest.VolStep,'g',6)+tr(" V"));
                }
                if(m_Ch[2].checked==1)
                {
                 ui->textBrowser->append(tr("VC:")+QString::number(m_Ch[2].Amp+A_SumStepnum*cAnytest.VolStep,'g',6)+tr(" V"));
                }

                if(VariatyVz2==0&&m_Ch[3].checked==1)
                  ui->textBrowser->append(tr("Vz:")+QString::number(m_Ch[3].Amp+A_SumStepnum*cAnytest.VolStep,'g',6)+tr(" V"));
                ...
                if(m_Ch[4].checked==1)
                {
                 ui->textBrowser->append(tr("IA:")+QString::number(m_Ch[4].Amp+A_SumStepnum*cAnytest.CurStep,'g',6)+tr(" A"));
                }
                if(m_Ch[5].checked==1)
                {
                 ui->textBrowser->append(tr("IB:")+QString::number(m_Ch[5].Amp+A_SumStepnum*cAnytest.CurStep,'g',6)+tr(" A"));
                }
                if(m_Ch[6].checked==1)
                {
                 ui->textBrowser->append(tr("IC:")+QString::number(m_Ch[6].Amp+A_SumStepnum*cAnytest.CurStep,'g',6)+tr(" A"));
                }
            ...
            case 3: //频率
                if(m_Ch[0].checked==1)
                {
                 ui->textBrowser->append(tr("VA:")+QString::number(m_Ch[0].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
                }
                if(m_Ch[1].checked==1)
                {
                 ui->textBrowser->append(tr("VB:")+QString::number(m_Ch[1].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
                }
                if(m_Ch[2].checked==1)
                {
                 ui->textBrowser->append(tr("VC:")+QString::number(m_Ch[2].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
                }

                if(VariatyVz2==0&&m_Ch[3].checked==1)
                  ui->textBrowser->append(tr("Vz:")+QString::number(m_Ch[3].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));



                if(m_Ch[4].checked==1)
                {
                 ui->textBrowser->append(tr("IA:")+QString::number(m_Ch[4].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
                }
                if(m_Ch[5].checked==1)
                {
                 ui->textBrowser->append(tr("IB:")+QString::number(m_Ch[5].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
                }
                if(m_Ch[6].checked==1)
                {
                 ui->textBrowser->append(tr("IC:")+QString::number(m_Ch[6].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
                }
                break;
            }
            ...
            testStr = "TripTime="+QString::number(TripTime,'g',6);
            testStr = ";ChangeSteps="+QString::number(A_SumStepnum);
            emit SIG_SendTestResult(testStr);
        }

        else if(cAnytest.TestMode==1)
        {
...
//       QString testStr = ui->textBrowser->toPlainText();
//        emit SIG_SendTestResult(testStr);
           ...
           if(m_Ch[4].Freq==0)   //是否直流
            {
              ...
              if(cAnytest.CurStep<MIN_I_VALUEDATA_DC)
              {
                  cAnytest.CurStep=MIN_I_VALUEDATA_DC;
                  ui->lineEdit_19->setText(QString::number(cAnytest.CurStep,'g',4));
              }
              else if(cAnytest.CurStep>30/*MAX_I_VALUEDATA_DC*/)
              {
                  cAnytest.CurStep=30/*MAX_I_VALUEDATA_DC*/;
                  ui->lineEdit_19->setText(QString::number(cAnytest.CurStep,'g',4));
              }
            }
          else
            {
              ...
              if(cAnytest.CurStep<MIN_I_VALUEDATA_AC)
              {
                  cAnytest.CurStep=MIN_I_VALUEDATA_AC;
                  ui->lineEdit_19->setText(QString::number(cAnytest.CurStep,'g',4));
              }
              else if(cAnytest.CurStep>MAX_I_VALUEDATA_AC)
              {
                  cAnytest.CurStep=MAX_I_VALUEDATA_AC;
                  ui->lineEdit_19->setText(QString::number(cAnytest.CurStep,'g',4));
              }
            }
          break;
 ...
 case 18:  //I3棰戠巼
    ...
    case 19:
           ...
           switch(m_Ch[4].DC)
             {
               case 0:  //交流
                 if(cAnytest.CurStep>MAX_I_VALUEDATA_AC)
                   cAnytest.CurStep=MAX_I_VALUEDATA_AC;
                 else if(cAnytest.CurStep<0)
                   cAnytest.CurStep=0;
                 break;
               case 1:  //鐩存祦鏃剁數娴佹闀块檺瀹氬€?
                 if(cAnytest.CurStep>30/*MAX_I_VALUEDATA_DC*/)
                   cAnytest.CurStep=30/*MAX_I_VALUEDATA_DC*/;
                 else if(cAnytest.CurStep<0)
                   cAnytest.CurStep=0;
                 break;
               ...
               switch(m_Ch[4].DC)
                 {
                   case 0:  //交流
                     if(cAnytest.CurStep>MAX_I_VALUEDATA_AC)
                       cAnytest.CurStep=MAX_I_VALUEDATA_AC;
                     else if(cAnytest.CurStep<0-MAX_I_VALUEDATA_AC)
                       cAnytest.CurStep=0-MAX_I_VALUEDATA_AC;
                     break;
                   case 1:  //鐩存祦鏃剁數娴佹闀块檺瀹氬€?
                     if(cAnytest.CurStep>MAX_I_VALUEDATA_DC)
                       cAnytest.CurStep=MAX_I_VALUEDATA_DC;
                     else if(cAnytest.CurStep<0-MAX_I_VALUEDATA_DC)
                       cAnytest.CurStep=0-MAX_I_VALUEDATA_DC;
                     break;
                 }
           }
            break;
    ...
    case 20:
           if(cAnytest.TestMode==1)
           {
            ...
            case 0:  //浜ゆ祦鏃剁數娴佹闀块檺瀹氬€?
              if(cAnytest.VolStep>MAX_V_VALUEDATA_AC)
                cAnytest.VolStep=MAX_V_VALUEDATA_AC;
              else if(cAnytest.VolStep<0)
                cAnytest.VolStep=0;
              break;
            case 1:  //鐩存祦鏃剁數娴佹闀块檺瀹氬€?
              if(cAnytest.VolStep>MAX_V_VALUEDATA_DC)
                cAnytest.VolStep=MAX_V_VALUEDATA_DC;
              else if(cAnytest.VolStep<0)
                cAnytest.VolStep=0;
              break;
            }
           ...
           if(cAnytest.TestMode==3)
           {
            ...
            case 0:  //浜ゆ祦鏃剁數娴佹闀块檺瀹氬€?
              if(cAnytest.VolStep>MAX_V_VALUEDATA_AC)
                cAnytest.VolStep=MAX_V_VALUEDATA_AC;
              else if(cAnytest.VolStep<0-MAX_V_VALUEDATA_AC)
                cAnytest.VolStep=0-MAX_V_VALUEDATA_AC;
              break;
            case 1:  //鐩存祦鏃剁數娴佹闀块檺瀹氬€?
              if(cAnytest.VolStep>MAX_V_VALUEDATA_DC)
                cAnytest.VolStep=MAX_V_VALUEDATA_DC;
              else if(cAnytest.VolStep<0-MAX_V_VALUEDATA_DC)
                cAnytest.VolStep=0-MAX_V_VALUEDATA_DC;
              break;
            }
           }
            //鏇存柊鏄剧ず鐢靛帇姝ラ暱
    ...
    case 21: //鐩镐綅姝ラ暱
           if(cAnytest.TestMode==1)
           {
                    if(cAnytest.PhaStep>180)
                      cAnytest.PhaStep=180;
                    else if(cAnytest.PhaStep<0)
                      cAnytest.PhaStep=0;
           }
           if(cAnytest.TestMode==3)
           {
    ...
    case 23:
                             //姝ラ暱鏃堕棿
                             if(cAnytest.StepTime>99.99)
                               cAnytest.StepTime=99.99;
                             else if(cAnytest.StepTime<0)
                               cAnytest.StepTime=0.5;
                             //鏇存柊鐣岄潰鏄剧ず
                             break;
    case 24:
                             if(cAnytest.StepNum>1000)
                               cAnytest.StepNum=1000;
                             else if(cAnytest.StepNum<1)
                               cAnytest.StepNum=1;
                             //鏇存柊鐣岄潰鏄剧ず
                             break;
    case 25:
                             if(cAnytest.PreFaultTime>999.9)
                              cAnytest.PreFaultTime=999.9;
                             else if(cAnytest.PreFaultTime<0)
                             cAnytest.PreFaultTime=0;
                            //鏇存柊鐣岄潰鏄剧ず
                              break;
    case 26:
             if(cAnytest.FaultTime>999.9)
             cAnytest.FaultTime=999.9;
             else if(cAnytest.FaultTime<0)
             cAnytest.FaultTime=0;
             //鏇存柊鐣岄潰鏄剧ず
              break;

    case 27:
            if(cAnytest.PostFaultTime>999.9)
                cAnytest.PostFaultTime=999.9;
            else if(cAnytest.PostFaultTime<0)
                cAnytest.PostFaultTime=0;
                //鏇存柊鐣岄潰鏄剧ず
               break;
    case 28:
...
void Manualtest::displaydataandstate(int position)
{
    ...
    if(position>=1&&position<=31)
    {
        ...
        case 19:lineedit->setText(QString::number(cAnytest.CurStep,'g',6)); break;
        case 20:lineedit->setText(QString::number(cAnytest.VolStep,'g',6)); break;
        case 21:lineedit->setText(QString::number(cAnytest.PhaStep,'g',6)); break;
        case 22:lineedit->setText(QString::number(cAnytest.FreStep,'g',6)); break;
        case 23:lineedit->setText(QString::number(cAnytest.StepTime,'g',6)); break;
        case 24:lineedit->setText(QString::number(cAnytest.StepNum,'g',6)); break;
        case 25:lineedit->setText(QString::number(cAnytest.PreFaultTime,'g',6)); break;
        case 26:lineedit->setText(QString::number(cAnytest.FaultTime,'g',6)); break;
        case 27:lineedit->setText(QString::number(cAnytest.PostFaultTime,'g',6)); break;
 ...
 void  Manualtest::updateVz()
{
     ...
     if(m_Ch[0].Freq==cAnytest.SysFrec&&m_Ch[1].Freq==cAnytest.SysFrec&&m_Ch[2].Freq==cAnytest.SysFrec&&VariatyVz2==1)
     {
        ...
        switch(cAnytest.Variable)
          {
            case 1://幅值
              if(m_Ch[0].checked==1)//VA选择
                {
                  m_Ch[0].Amp=m_Ch[0].Amp+cAnytest.VolStep*devision;
                  switch(m_Ch[0].DC)
                  {
                    case 0:  //VA  交流
                      if(m_Ch[0].Amp>MAX_V_VALUEDATA_AC)
              ...
              if(m_Ch[1].checked==1)
                {
                  m_Ch[1].Amp=m_Ch[1].Amp+cAnytest.VolStep*devision;
                  switch(m_Ch[1].DC)
                  {
                    case 0:  //VA  交流
                      if(m_Ch[1].Amp>MAX_V_VALUEDATA_AC)
              ...
              if(m_Ch[2].checked==1)
                {//V3瑕佸彉鍖?                  
                  m_Ch[2].Amp=m_Ch[2].Amp+cAnytest.VolStep*devision;
                  switch(m_Ch[2].DC)
                  {
                    case 0:  //VA  交流
                      if(m_Ch[2].Amp>MAX_V_VALUEDATA_AC)
              ...
              if(m_Ch[3].checked==1)
                {//V3瑕佸彉鍖?
                  m_Ch[3].Amp=m_Ch[3].Amp+cAnytest.VolStep*devision;
                  switch(m_Ch[3].DC)
                  {
                    case 0:  //VA  交流
                      if(m_Ch[3].Amp>MAX_V_VALUEDATA_AC)
              ...
              if(m_Ch[4].checked==1)
                {//I1瑕佸彉鍖?
                  m_Ch[4].Amp=m_Ch[4].Amp+cAnytest.CurStep*devision;
                  switch(m_Ch[4].DC)
                  {
                    case 0:  //IA  交流
                      if(m_Ch[4].Amp>MAX_I_VALUEDATA_AC)
              ...
              if(m_Ch[5].checked==1)
                {//I2瑕佸彉鍖?
                  m_Ch[5].Amp=m_Ch[5].Amp+cAnytest.CurStep*devision;
                  switch(m_Ch[5].DC)
                  {
                    case 0:  //IA  交流
                      if(m_Ch[5].Amp>MAX_I_VALUEDATA_AC)
              ...
              if(m_Ch[6].checked==1)
                {//I3瑕佸彉鍖?
                  m_Ch[6].Amp=m_Ch[6].Amp+cAnytest.CurStep*devision;
                  switch(m_Ch[6].DC)
                  {
                    case 0:  //IA  交流
                      if(m_Ch[6].Amp>MAX_I_VALUEDATA_AC)
            ...
            case 2:
              ...
              if(m_Ch[0].checked==1&&m_Ch[0].DC==0)
                {
                  m_Ch[0].Phase=m_Ch[0].Phase+cAnytest.PhaStep*devision;
                  if(m_Ch[0].Phase>180)
                    {
                      m_Ch[0].Phase=m_Ch[0].Phase-cAnytest.PhaStep*devision;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        V1RelaPhase=cAnytest.PhaStep*devision;
                      else if(cAnytest.PhaStep<0)
                        V1RelaPhase=360+cAnytest.PhaStep*devision;
                  }
              ...
              if(m_Ch[1].checked==1&&m_Ch[1].DC==0)
                {//V2瑕佸彉鍖?
                  m_Ch[1].Phase=m_Ch[1].Phase+cAnytest.PhaStep*devision;
                  if(m_Ch[1].Phase>180)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[1].Phase=m_Ch[1].Phase-cAnytest.PhaStep*devision;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        V2RelaPhase=cAnytest.PhaStep*devision;
                      else if(cAnytest.PhaStep<0)
                        V2RelaPhase=360+cAnytest.PhaStep*devision;
                  }
              ...
              if(m_Ch[2].checked==1&&m_Ch[2].DC==0)
                {//V3瑕佸彉鍖?
                  m_Ch[2].Phase=m_Ch[2].Phase+cAnytest.PhaStep*devision;
                  if(m_Ch[2].Phase>180)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[2].Phase=m_Ch[2].Phase-cAnytest.PhaStep*devision;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        V3RelaPhase=cAnytest.PhaStep*devision;
                      else if(cAnytest.PhaStep<0)
                        V3RelaPhase=360+cAnytest.PhaStep*devision;
                  }
              ...
              if(m_Ch[3].checked==1&&m_Ch[3].DC==0)
                {
                  m_Ch[3].Phase=m_Ch[3].Phase+cAnytest.PhaStep*devision;
                  if(m_Ch[3].Phase>180)
                    {
                      m_Ch[3].Phase=m_Ch[3].Phase-cAnytest.PhaStep*devision;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        VzRelaPhase=cAnytest.PhaStep*devision;
                      else if(cAnytest.PhaStep<0)
                        VzRelaPhase=360+cAnytest.PhaStep*devision;
                  }
              ...
              if(m_Ch[4].checked==1&&m_Ch[4].DC==0)
                {//I1瑕佸彉鍖?
                  m_Ch[4].Phase=m_Ch[4].Phase+cAnytest.PhaStep*devision;
                  if(m_Ch[4].Phase>180)
                    {
                      m_Ch[4].Phase=m_Ch[4].Phase-cAnytest.PhaStep*devision;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        I1RelaPhase=cAnytest.PhaStep*devision;
                      else if(cAnytest.PhaStep<0)
                        I1RelaPhase=360+cAnytest.PhaStep*devision;
                  }
              ...
              if(m_Ch[5].checked==1&&m_Ch[5].DC==0)
                {//I2瑕佸彉鍖?
                  m_Ch[5].Phase=m_Ch[5].Phase+cAnytest.PhaStep*devision;
                  if(m_Ch[5].Phase>180)
                    {
                      m_Ch[5].Phase=m_Ch[5].Phase-cAnytest.PhaStep*devision;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        I2RelaPhase=cAnytest.PhaStep*devision;
                      else if(cAnytest.PhaStep<0)
                        I2RelaPhase=360+cAnytest.PhaStep*devision;
                  }
              ...
              if(m_Ch[6].checked==1&&m_Ch[6].DC==0)
                {//I3瑕佸彉鍖?
                  m_Ch[6].Phase=m_Ch[6].Phase+cAnytest.PhaStep*devision;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        I3RelaPhase=cAnytest.PhaStep*devision;
                      else if(cAnytest.PhaStep<0)
                        I3RelaPhase=360+cAnytest.PhaStep*devision;
                  }
            ...
            case 3:
              if(m_Ch[0].checked==1&&m_Ch[0].DC==0)
                {//V1瑕佸彉鍖?
                  m_Ch[0].Freq=m_Ch[0].Freq+cAnytest.FreStep*devision;
                  if(m_Ch[0].Freq>1000)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[0].Freq=m_Ch[0].Freq-cAnytest.FreStep*devision;
                      flag=1;
              ...
              if(m_Ch[1].checked==1&&m_Ch[1].DC==0)
                {//V2瑕佸彉鍖?
                  m_Ch[1].Freq=m_Ch[1].Freq+cAnytest.FreStep*devision;
                  if(m_Ch[1].Freq>1000)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[1].Freq=m_Ch[1].Freq-cAnytest.FreStep*devision;
                      flag=1;
              ...
              if(m_Ch[2].checked==1&&m_Ch[2].DC==0)
                {//V3瑕佸彉鍖?
                  m_Ch[2].Freq=m_Ch[2].Freq+cAnytest.FreStep*devision;
                  if(m_Ch[2].Freq>1000)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[2].Freq=m_Ch[2].Freq-cAnytest.FreStep*devision;
                      flag=1;
              ...
              if(m_Ch[3].checked==1&&m_Ch[3].DC==0)
                {//V3瑕佸彉鍖?
                  m_Ch[3].Freq=m_Ch[3].Freq+cAnytest.FreStep*devision;
                  if(m_Ch[3].Freq>1000)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[3].Freq=m_Ch[3].Freq-cAnytest.FreStep*devision;
                      flag=1;
              ...
              if(m_Ch[4].checked==1&&m_Ch[4].DC==0)
                {//I1瑕佸彉鍖?
                  m_Ch[4].Freq=m_Ch[4].Freq+cAnytest.FreStep*devision;
                  if(m_Ch[4].Freq>1000)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[4].Freq=m_Ch[4].Freq-cAnytest.FreStep*devision;
                      flag=1;
              ...
              if(m_Ch[5].checked==1&&m_Ch[5].DC==0)
                {//I2瑕佸彉鍖?
                  m_Ch[5].Freq=m_Ch[5].Freq+cAnytest.FreStep*devision;
                  if(m_Ch[5].Freq>1000)
              ...
              if(m_Ch[6].checked==1&&m_Ch[6].DC==0)
                {//I3瑕佸彉鍖?
                  m_Ch[6].Freq=m_Ch[6].Freq+cAnytest.FreStep*devision;
                  if(m_Ch[6].Freq>1000)
                ...
                {//V1瑕佸彉鍖?
                   ...
                   else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        V1RelaPhase=0-cAnytest.PhaStep*devision;
                  }
                    displaydataandstate(2);
                }
              else if(m_Ch[0].checked!=1&&m_Ch[0].DC==0)
                ...
                {//V2瑕佸彉鍖?
                  ...
                  else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        V2RelaPhase=0-cAnytest.PhaStep*devision;
                  }
                   displaydataandstate(5);
                }
              else if(m_Ch[1].checked!=1&&m_Ch[1].DC==0)
                ...
                {//V3瑕佸彉鍖?
                  ...
                  else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        V3RelaPhase=0-cAnytest.PhaStep*devision;
                  }
                  displaydataandstate(8);
                }
              else if(m_Ch[2].checked!=1&&m_Ch[2].DC==0)
                ...
                {//V3瑕佸彉鍖?
                  ...
                  else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        VzRelaPhase=0-cAnytest.PhaStep*devision;
                  }
                  displaydataandstate(30);
                }
              else if(m_Ch[3].checked!=1&&m_Ch[3].DC==0)
                ...
                {//I1瑕佸彉鍖?
                  ...
                  else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        I1RelaPhase=0-cAnytest.PhaStep*devision;
                  }
                 displaydataandstate(11);
                }
              else if(m_Ch[4].checked!=1&&m_Ch[4].DC==0)
                ...
                {//I2瑕佸彉鍖?
                  ...
                  else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        I2RelaPhase=0-cAnytest.PhaStep*devision;
                  }
                  displaydataandstate(14);
                }
                ...
                {//I3瑕佸彉鍖?
                  ...
                  else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        I3RelaPhase=0-cAnytest.PhaStep*devision;
                  }
                   displaydataandstate(17);
                }
              else if(m_Ch[6].checked!=1&&m_Ch[6].DC==0)
...
//                                    1增加 0减小        1 电压 0电流
int Manualtest::vi_change_step_simple(int add_or_dec,int devision)//
  {
    if(ManulIsSendcmd==true)
        return 0;
    ...
    if(add_or_dec==1)
      {
        switch(cAnytest.Variable)
          {
            case 1://幅值
              if(m_Ch[0].checked==1&&devision==1)//VA选择
                {
                  m_Ch[0].Amp=m_Ch[0].Amp+cAnytest.VolStep;
                  switch(m_Ch[0].DC)
                  {
                    case 0:  //VA  交流
                      if(m_Ch[0].Amp>MAX_V_VALUEDATA_AC)
              ...
              if(m_Ch[4].checked==1&&devision==0)
                {//I1瑕佸彉鍖?
                  m_Ch[4].Amp=m_Ch[4].Amp+cAnytest.CurStep;
                  switch(m_Ch[4].DC)
                  {
                    case 0:  //IA  交流
                      if(m_Ch[4].Amp>MAX_I_VALUEDATA_AC)
            ...
            case 2:
              ...
              if(m_Ch[0].checked==1&&m_Ch[0].DC==0&&devision==1)
                {
                  m_Ch[0].Phase=m_Ch[0].Phase+cAnytest.PhaStep;
                  if(m_Ch[0].Phase>180)
                    {
                      m_Ch[0].Phase=m_Ch[0].Phase-cAnytest.PhaStep;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        V1RelaPhase=cAnytest.PhaStep;
                      else if(cAnytest.PhaStep<0)
                        V1RelaPhase=360+cAnytest.PhaStep;
                  }
                  displaydataandstate(2);
                }
              else if(m_Ch[0].checked!=1&&m_Ch[0].DC==0&&devision==1)
                {//V1涓嶅彉鍖?
                  V1RelaPhase=0;
                }

              if(m_Ch[4].checked==1&&m_Ch[4].DC==0&&devision==0)
                {//I1瑕佸彉鍖?
                  m_Ch[4].Phase=m_Ch[4].Phase+cAnytest.PhaStep;
                  if(m_Ch[4].Phase>180)
                    {
                      m_Ch[4].Phase=m_Ch[4].Phase-cAnytest.PhaStep;
                      flag=1;
                  ...
                  {
                      if(cAnytest.PhaStep>=0)
                        I1RelaPhase=cAnytest.PhaStep;
                      else if(cAnytest.PhaStep<0)
                        I1RelaPhase=360+cAnytest.PhaStep;
                  }
                  displaydataandstate(11);
                }
              else if(m_Ch[4].checked!=1&&m_Ch[4].DC==0&&devision==0)
                {
                  I1RelaPhase=0;
                }
             break;
            ...
            case 3:
              if(m_Ch[0].checked==1&&m_Ch[0].DC==0&&devision==1)
                {//V1瑕佸彉鍖?
                  m_Ch[0].Freq=m_Ch[0].Freq+cAnytest.FreStep;
                  if(m_Ch[0].Freq>1000)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[0].Freq=m_Ch[0].Freq-cAnytest.FreStep;
                      flag=1;
              ...
              if(m_Ch[4].checked==1&&m_Ch[4].DC==0&&devision==0)
                {//I1瑕佸彉鍖?
                  m_Ch[4].Freq=m_Ch[4].Freq+cAnytest.FreStep;
                  if(m_Ch[4].Freq>1000)
                    {//濡傛灉澧炲姞鍒版渶澶у€煎垯涓嶅彉
                      m_Ch[4].Freq=m_Ch[4].Freq-cAnytest.FreStep;
                      flag=1;
    ...
    else /*if(increase==0)*/
      {
        switch(cAnytest.Variable)
          {
            case 1://鍙橀噺鏄箙鍊?
              if(m_Ch[0].checked==1&&devision==1)
                {//V1瑕佸彉鍖?
                  m_Ch[0].Amp=m_Ch[0].Amp-cAnytest.VolStep;
                  switch(m_Ch[0].DC)
                  {
                ...
                {//V1瑕佸彉鍖?
                   ...
                   else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        V1RelaPhase=0-cAnytest.PhaStep;
                  }
                    displaydataandstate(2);
                }
              else if(m_Ch[0].checked!=1&&m_Ch[0].DC==0&&devision==1)
                ...
                {//I1瑕佸彉鍖?
                  ...
                  else
                  {
                      if(cAnytest.PhaStep>=0)
                        ...
                        I1RelaPhase=0-cAnytest.PhaStep;
                  }
                 displaydataandstate(11);
                }
              else if(m_Ch[4].checked!=1&&m_Ch[4].DC==0&&devision==0)
     ...
     else if(cAnytest.TestMode==3)
     {
       ...
       case 1:  //幅值
           if(m_Ch[0].checked==1)
           {
            ...
            QByteArray ba;
            char *buf;
            str=QString(tr("VA:")+QString::number(m_Ch[0].Amp+A_SumStepnum*(cAnytest.VolStep),'g',6)+tr(" V"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           ...
           if(m_Ch[1].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
               str=QString(tr("VB:")+QString::number(m_Ch[1].Amp+A_SumStepnum*(cAnytest.VolStep),'g',6)+tr(" V"));
               ba=str.toLatin1();
               buf=ba.data();
               fprintf(fp,"%s\r\n",buf);
           ...
           if(m_Ch[2].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("VC:")+QString::number(m_Ch[2].Amp+A_SumStepnum*(cAnytest.VolStep),'g',6)+tr(" V"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           ...
           if(VariatyVz2==0&&m_Ch[3].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("Vz:")+QString::number(m_Ch[3].Amp+A_SumStepnum*(cAnytest.VolStep),'g',6)+tr(" V"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           ...
           if(m_Ch[4].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
               str=QString(tr("IA:")+QString::number(m_Ch[4].Amp+A_SumStepnum*(cAnytest.CurStep),'g',6)+tr(" A"));
               ba=str.toLatin1();
               buf=ba.data();
               fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[5].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
               str=QString(tr("IB:")+QString::number(m_Ch[5].Amp+A_SumStepnum*(cAnytest.CurStep),'g',6)+tr(" A"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[6].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("IC:")+QString::number(m_Ch[6].Amp+A_SumStepnum*(cAnytest.CurStep),'g',6)+tr(" A"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
       ...
       case 2: // 相位
           if(m_Ch[0].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("VA:")+QString::number(m_Ch[0].Phase+A_SumStepnum*cAnytest.PhaStep,'g',6)+tr(" Deg"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[1].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
               str=QString(tr("VB:")+QString::number(m_Ch[1].Phase+A_SumStepnum*cAnytest.PhaStep,'g',6)+tr(" Deg"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           ...
           {   QString str;
               QByteArray ba;
               char *buf;
            str=QString(tr("VC:")+QString::number(m_Ch[2].Phase+A_SumStepnum*cAnytest.PhaStep,'g',6)+tr(" Deg"));
               ba=str.toLatin1();
               buf=ba.data();
               fprintf(fp,"%s\r\n",buf);
           }
           ...
           {   QString str;
               QByteArray ba;
               char *buf;
            str=QString(tr("Vz:")+QString::number(m_Ch[3].Phase+A_SumStepnum*cAnytest.PhaStep,'g',6)+tr(" Deg"));
               ba=str.toLatin1();
               buf=ba.data();
               fprintf(fp,"%s\r\n",buf);
           }
           ...
           if(m_Ch[4].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("IA:")+QString::number(m_Ch[4].Phase+A_SumStepnum*cAnytest.PhaStep,'g',6)+tr(" Deg"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           ...
           if(m_Ch[5].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("IB:")+QString::number(m_Ch[5].Phase+A_SumStepnum*cAnytest.PhaStep,'g',6)+tr(" Deg"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[6].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
                str=QString(tr("IC:")+QString::number(m_Ch[6].Phase+A_SumStepnum*cAnytest.PhaStep,'g',6)+tr(" Deg"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
       ...
       case 3: //频率
           if(m_Ch[0].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
               str=QString(tr("VA:")+QString::number(m_Ch[0].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
               ba=str.toLatin1();
               buf=ba.data();
               fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[1].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("VB:")+QString::number(m_Ch[1].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[2].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("VC:")+QString::number(m_Ch[2].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           ...
           if(VariatyVz2==0&&m_Ch[3].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("Vz:")+QString::number(m_Ch[3].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           ...
           if(m_Ch[4].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("IA:")+QString::number(m_Ch[4].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[5].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("IB:")+QString::number(m_Ch[5].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
           if(m_Ch[6].checked==1)
           {
               ...
               QByteArray ba;
               char *buf;
            str=QString(tr("IC:")+QString::number(m_Ch[6].Freq+A_SumStepnum*cAnytest.FreStep,'g',6)+tr(" Hz"));
            ba=str.toLatin1();
            buf=ba.data();
            fprintf(fp,"%s\r\n",buf);
           }
...
void  Manualtest::ShowWarning(int Outtype,int type)
{
    ...
    else  if(cAnytest.Variable==3)//频率
    {
       if(current_language==1)
       {
        if(type==1)//超过
...
{
    if(cAnytest.TestMode==1||cAnytest.TestMode==3)
    {
...
void Manualtest::setlineedittext(QString str)
{
    ...
    case 19:
        cAnytest.CurStep=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number(cAnytest.CurStep,'g',6));
        break;
    case 20:
        cAnytest.VolStep=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number(cAnytest.VolStep,'g',6));
        break;
    case 21:
        cAnytest.PhaStep=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number(cAnytest.PhaStep,'g',6));
        break;
    case 22:
        cAnytest.FreStep=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number(cAnytest.FreStep,'g',6));
        break;
    case 23:
        cAnytest.StepTime=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number(cAnytest.StepTime,'g',6));
        break;
    case 24:
        cAnytest.StepNum=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number( cAnytest.StepNum,'g',6));
        break;
    case 25:
        cAnytest.PreFaultTime=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number( cAnytest.PreFaultTime,'g',6));
        break;
    case 26:
        cAnytest.FaultTime=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number(cAnytest.FaultTime,'g',6));
        break;
    case 27:
        cAnytest.PostFaultTime=number;
        vi_parameter_enter(currenteditpos);
        currentedit->setText(QString::number(cAnytest.PostFaultTime,'g',6));
        break;
    case 28:
...
{
    if(starttest==1&&cAnytest.TestMode==1)
        vi_change_step_simple(1,1);
    //vi_change_step(1,1);
    updateVz();
}
    ...
    else if(event->type()==QEvent::FocusOut)
    {
        ...
        else if(obj==ui->lineEdit_2)
        {
        ...
        else if(obj==ui->lineEdit_19)
         {
                ...
                cAnytest.CurStep= text.toFloat();
                currenteditpos=19;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_19->setText(QString::number(cAnytest.CurStep,'g',6));
         }
        else if(obj==ui->lineEdit_20)
         {
                QString text=ui->lineEdit_20->text();
                cAnytest.VolStep= text.toFloat();
                currenteditpos=20;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_20->setText(QString::number(cAnytest.VolStep,'g',6));
         }
        else if(obj==ui->lineEdit_21)
         {
                QString text=ui->lineEdit_21->text();
                cAnytest.PhaStep= text.toFloat();
                currenteditpos=21;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_21->setText(QString::number(cAnytest.PhaStep,'g',6));
         }
        else if(obj==ui->lineEdit_22)
         {
                QString text=ui->lineEdit_22->text();
                cAnytest.FreStep= text.toFloat();
                currenteditpos=22;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_22->setText(QString::number(cAnytest.FreStep,'g',6));
         }
        else if(obj==ui->lineEdit_23)
         {
                QString text=ui->lineEdit_23->text();
                cAnytest.StepTime= text.toFloat();
                currenteditpos=23;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_23->setText(QString::number(cAnytest.StepTime,'g',6));
         }
        else if(obj==ui->lineEdit_24)
         {
                QString text=ui->lineEdit_24->text();
                cAnytest.StepNum= text.toFloat();
                currenteditpos=24;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_24->setText(QString::number(cAnytest.StepNum,'g',6));
         }
        else if(obj==ui->lineEdit_25)
         {
                QString text=ui->lineEdit_25->text();
                cAnytest.PreFaultTime= text.toFloat();
                currenteditpos=25;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_25->setText(QString::number(cAnytest.PreFaultTime,'g',6));
         }
        else if(obj==ui->lineEdit_26)
         {
                QString text=ui->lineEdit_26->text();
                cAnytest.FaultTime= text.toFloat();
                currenteditpos=26;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_26->setText(QString::number(cAnytest.FaultTime,'g',6));
         }
        else if(obj==ui->lineEdit_27)
         {
                QString text=ui->lineEdit_27->text();
                cAnytest.PostFaultTime= text.toFloat();
                currenteditpos=27;
                vi_parameter_enter(currenteditpos);
                ui->lineEdit_27->setText(QString::number(cAnytest.PostFaultTime,'g',6));
         }
        else if(obj==ui->lineEdit_28)
         {
...
void Manualtest::on_checkBox_8_clicked()
{
    if(VariatyVz2!=1)  //
    {
        if(m_Ch[0].Freq==cAnytest.SysFrec&&m_Ch[1].Freq==cAnytest.SysFrec&&m_Ch[2].Freq==cAnytest.SysFrec)
      {
...
void Manualtest::GetTestButtenstate(){
    if(starttest==1)
    {
        ui->pushButton_6->setText(tr("停止"));
        ui->pushButton_6->setStyleSheet("QPushButton{font-family:'wenquanyi';font-size:16px;background-color:rgb(150,255,255);color:black;border-radius:2px;border:2px groove gray;border-style:outset;}");
...
void Manualtest::on_checkBox_10_clicked()
{
...
void Manualtest::on_checkBox_11_clicked()
{
...
void Manualtest::on_pushButton_7_clicked()
{
    if(starttest==1&&cAnytest.TestMode==1)
     vi_change_step_simple(1,0);
    updateVz();
}

void Manualtest::on_pushButton_8_clicked()
{
    if(starttest==1&&cAnytest.TestMode==1)
     vi_change_step_simple(0,0);
    updateVz();
}

...
double my_atof(unsigned char *s)
{
    double power,value;
    int i,sign;
    assert(s!=NULL);//脜脨露脧脳脰路没沤庐脢脟路帽脦陋驴脮
    for(i=0;is_space(s[i]);i++);//鲁媒脠楼脳脰路没沤庐脟掳碌脛驴脮啪帽
    sign=(s[i]=='-')?-1:1;
    if(s[i]=='-'||s[i]=='+')//脪陋脢脟脫脨路没潞脜脦禄鸥脥脟掳艙酶脪禄脦禄
        i++;
    for(value=0.0;is_digit(s[i]);i++)//艗脝脣茫脨隆脢媒碌茫脟庐碌脛脢媒脳脰
        value=value*10.0+(s[i]-'0');
    if(s[i]=='.')
        i++;
    for(power=1.0;is_digit(s[i]);i++)//艗脝脣茫脨隆脢媒碌茫潞贸碌脛脢媒脳脰
    {
        value=value*10.0+(s[i]-'0');
        power*=10.0;
    }
...
/*-----------------------------------------------------------------------------
  潞炉脢媒脙没:      serial_read
  虏脦脢媒:        int fd,char *str,unsigned int len,unsigned int timeout
  路碌禄脴脰碌:      脭脷鹿忙露拧碌脛脢卤艗盲脛脷露脕脠隆脢媒鸥脻拢卢鲁卢脢卤脭貌脥脣鲁枚拢卢鲁卢脢卤脢卤艗盲脦陋ms艗露卤冒
  脙猫脢枚:        脧貌fd脙猫脢枚路没碌脛沤庐驴脷艙脫脢脮脢媒鸥脻拢卢鲁鈧睹埫幝猯en拢卢沤忙脠毛str拢卢timeout 脦陋鲁卢脢卤脢卤艗盲
 *-----------------------------------------------------------------------------*/


int serial_read(int fd, unsigned char *str, unsigned int len,unsigned char *cmp_data,int cmp_len)
{
        int ret;		//脙驴沤脦露脕碌脛艙谩鹿没
        int sret;		//select艗脿驴脴艙谩鹿没
        int readlen = 0;	//脢碌艗脢露脕碌艙碌脛脳脰艙脷脢媒
        unsigned char * ptr;
...
#ifdef ARMLINUX
//check_f 脢脟路帽脨猫脪陋脨拢脩茅  脰梅脪陋脨猫脪陋啪脛脮芒啪枚潞炉脢媒
int receive_send(int fd,unsigned char *Send_str,int Send_len,unsigned char *cmp_data,int Receive_len,int cmp_len,int check_f)
  {
    ...
    if(Send_len!=0)
         ...
         //    write(fd,Send_str,Send_len); //路垄脣脥脳脰路没鲁枚脠楼
             *(Send_str+Send_len)=send_checksum;
          //   *(Send_str+Send_len)=send_checksum;
            write(fd,Send_str,Send_len+1); //路垄脣脥脳脰路没鲁枚脠楼

          //   write(fd,&send_checksum,1);  //路垄脣脥脨拢脩茅脗毛
          }
        ...
        if(Receive_len!=0)
          {
           //  delayus(100);
             ...
             if(reclen==(Receive_len+check_f))
              {
               if(memcmp(cmp_data,receive_data,cmp_len)==0) //脠莽鹿没卤脠艙脧碌脛脳脰路没虏禄露脭 脭貌路碌禄脴-1
                  return  1;
                else return -1;
...
int transmit(int fd, unsigned char *buffer,int length, int length2)
  {
    ...
    if(length2)
      ...
      re_len=5;
    cmp_len=5;
    send_data1[4]=0x00; //脮芒脌茂脢脟路帽脫脨脦脢脤芒
    buffer=buffer+4;
    for(i=0;i<length-4;i++)
      {
        send_data1[5+i]=*buffer;  // 路垄脣脥碌脛脢媒鸥脻send_data1
        buffer++;
      }
    ...
    while(1)
       {
         ...
         if(i==1)
           {
             for(i=0;i<length2;i++)
             result[i]=receive_data[5+i]; //receive_data 脟掳5脦禄脢脟脫艩沤冒脨脜潞脜
...
void artExec(int fd,  char *command_str,unsigned char *result1,int max_result_length)
    {
      ...
      if(memcmp(p1,"sys:",4)==0)     //system commands
           ...
           transmit(fd,sys_data,length,max_result_length);
            ...
            else if(memcmp(p1,"dig:interval(",13)==0)  //驴陋鹿脴脕驴路脰卤忙脗脢隆拢
                 {
                   p1=p1+13;
                   inp_data[5]=0x07;
                   inp_data[6]=0x2d;
                   ...
                   inp_data[9]=(unsigned char)(time_out); //脭脷脧碌脥鲁脡猫脰脙脌茂脭枚艗脫隆掳驴陋鹿脴脕驴路脰卤忙脗脢隆卤潞贸
                  // inp_data[9]=0x3c;  //3ms mxg 2003_3_4
                   inp_data[10]=0x01;  //0x01拢潞虏禄卤拢沤忙艙脫碌茫露露露炉脢媒鸥脻拢卢0x00:卤拢沤忙艙脫碌茫露露露炉脢媒鸥脻隆拢
                   length=11;
                   transmit(fd,inp_data,length,max_result_length);
                 }
            else if(memcmp(p1,"cfg?",4)==0)
                ...
                length=10;
             ...
             else if(memcmp(p1,"wait(",5)==0)
                             {
                              ...
                              if((seq_data[9]&0x0f)==0)
                               ...
                               data1=p1[0]+1;  //绉诲姩鍒orbin(1),bin(1),bin(1),bin(1),2,1,0,1) 涓�

                               seq_data[7]=seq_data[7]|((data1&0x03)<<4);
                               if(p1[2]=='1'||p1[2]=='2')    //寤舵椂鏈夋晥
                               {    if(p1[2]=='1')
                                  {
...
void channel_input1(int fd,int type,double value, double phan,double frec,int sin_dc_status1,int channel_status,double value_step,double phase_step,double frec_step)
  {
    ...
    if(phan<0)
      phan+=360;//2007-10-08,相位解释为无符号�所以要�180�80转化��60之间.
    for(i=0;p1[i]!=NULL;i++)
      buffer4[i]=p1[i];
    buffer4[i]=NULL;
    buffer1[0]=NULL;
        ...
        if(channel_status==1)
          {
            ...
            strcpy(p1,"out:ana:v(1:1):sig1:");
            if (phan!=360)
               phan+=VaPhaseRevision;//07-10-08 16:10,脧脿脦禄脨拢脳艗
          }
        else if(channel_status==2)
          {
            for(i=0;i<40;i++)
            ...
            if (phan!=360)  //2008-12-25
               phan+=VbPhaseRevision;//07-10-08 16:10,脧脿脦禄脨拢脳艗
          }
        else if(channel_status==3)
          {
            for(i=0;i<40;i++)
            ...
            strcpy(p1,"out:ana:v(1:3):sig1:");
            if(phan!=360)  //2008-12-25
               phan+=VcPhaseRevision;//07-10-08 16:10,脧脿脦禄脨拢脳艗
          }
        else if(channel_status==4)
      {
                    for(i=0;i<40;i++)
                    ...
                    if(phan!=360)  //2008-12-25
                       phan+=VzPhaseRevision;//07-10-08 16:10,鐩镐綅鏍″噯
      }
        ...
        if(channel_status==1)
          {
            ...
            strcpy(p1,"out:ana:i(1:1):sig1:");
            if(phan!=360)  //2008-12-25
               phan+=IaPhaseRevision;//07-10-08 16:10,脧脿脦禄脨拢脳艗
          }
        else if(channel_status==2)
          {
            for(i=0;i<40;i++)
            ...
            strcpy(p1,"out:ana:i(1:2):sig1:");
            if(phan!=360)  //2008-12-25
               phan+=IbPhaseRevision;//07-10-08 16:10,脧脿脦禄脨拢脳艗
          }
        else if(channel_status==3)
          {
            for(i=0;i<40;i++)
            ...
            strcpy(p1,"out:ana:i(1:3):sig1:");
            if(phan!=360)  //2008-12-25
               phan+=IcPhaseRevision;//07-10-08 16:10,脧脿脦禄脨拢脳艗
          }
      }
    if(phan<0)
      phan+=360;
...
int read_max(int fd)
 {
   ...
   artExec(fd,"others:revision:get",result,59);//露脕脝碌脗脢隆垄脧脿脦禄虏鹿鲁楼虏脦脢媒
   response[0]=result[7];
   response[1]=result[8];
   response[2]=result[9];
   FreqRevision=BytesToFloat(response,4,3,SIGNED);//脝碌脗脢虏鹿鲁楼虏脦脢媒

   response[0]=result[10];
   response[1]=result[11];
   response[2]=result[12];
   VaPhaseRevision=BytesToFloat(response,4,3,SIGNED);//Va脧脿脦禄虏鹿鲁楼虏脦脢媒
   response[0]=result[13];
   response[1]=result[14];
   response[2]=result[15];

   VbPhaseRevision=BytesToFloat(response,4,3,SIGNED);//Vb脧脿脦禄虏鹿鲁楼虏脦脢媒
   response[0]=result[16];
   response[1]=result[17];
   response[2]=result[18];
   VcPhaseRevision=BytesToFloat(response,4,3,SIGNED);//Vc脧脿脦禄虏鹿鲁楼虏脦脢媒

     response[0]=result[19];
      response[1]=result[20];
      response[2]=result[21];
      VzPhaseRevision=BytesToFloat(response,4,3,SIGNED);//Vz鐩镐綅琛ュ伩鍙傛�
   ...
   IaPhaseRevision=BytesToFloat(response,4,3,SIGNED);//Ia脧脿脦禄虏鹿鲁楼虏脦脢媒
   response[0]=result[25];
   response[1]=result[26];
   response[2]=result[27];
   IbPhaseRevision=BytesToFloat(response,4,3,SIGNED);//Ib脧脿脦禄虏鹿鲁楼虏脦脢媒
   response[0]=result[28];
   response[1]=result[29];
   response[2]=result[30];
   IcPhaseRevision=BytesToFloat(response,4,3,SIGNED);//Ic脧脿脦禄虏鹿鲁楼虏脦脢媒

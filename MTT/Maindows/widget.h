...
public:
    ...
    CDevilist m_SerialServer;
    void INcallBackExample(TypeOfCOM typeofcom,int Value, string& notice);
    void initFromPara();
    thread1 *uartthread;
    QMap<QString,int> p_widget;
    QMap<QString,QMap<QString,float> > mTestPara;
    //CWindowsBase* p_currentModelWidget;
    CWindowsBase* p_currentModelWidget;

    bool OpenTestWidget(int index);
protected slots:
    void sysinit();
    void moveEvent(QMoveEvent *);
    void sendDSPstate(int sate);
    void sendDSPstate();
    void sendTestResult(QString strResult);
    void showDspInfo();
  //  void OnMouse();
    void  onCloseFrom();

    void onProcessPara(QString notice);

signals:
   void SIG_processPara(QString notice);
   void SIG_SendIndexToThread(int index);

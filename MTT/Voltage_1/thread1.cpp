...
{ 
       ...
       if(t2==1)
       {
        ...
        case 1:
            if(VStart>VStop)
            ChangeValue=VStart-(VStep/*-V1ampPerStepError*/)*stepnum;
            else
            ChangeValue=VStart+(VStep/*-V1ampPerStepError*/)*stepnum;
           ...
           switch(seq_count)
             ...
             case 3:
               ...
               case 4:
                 ...
                 dvdttripvalue=currentdvdt+dvdtStep+dvdtStep;
                 else
                 dvdttripvalue=currentdvdt-dvdtStep-dvdtStep;

                 break;
               case 5:
                 TripTime5=SequencerTime4/*-prefaulttime*/-SequencerTime2-timeTime-faultduration-LogicResolution/1000;
                 Temp=TripTime5*1000;
                 Temp=ceil(Temp);
                 TripTime5=Temp/1000;//将返回值的限制在小数点后最多只有两位
                 //DisplayResult(5);
                 break;
             }
         }
           ...
           switch(seq_count)
             {
               case 1:
                ...
                {
                  if(VStart>VStop)
                   TripValue=VStart-TripValueSteps*(VStep/*-V1ampPerStepError*/);
                 else
                   TripValue=VStart+TripValueSteps*(VStep/*-V1ampPerStepError*/);
                   DisplayResult(1);
                   if(TestItem2==1)
                   {
                     if(ReturnSteps<=0)
                       ...
                       TripTime2=1;
                     if(VStart>VStop)
                       ReturnValue=VStart-ReturnSteps*(VStep/*-V1ampPerStepError*/);
                     else
                       ReturnValue=VStart+ReturnSteps*(VStep/*-V1ampPerStepError*/);
                     DisplayResult(2);
                   }

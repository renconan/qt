    ...
    ui(new Ui::Widget)
{
  ...
  this->setWindowFlags(Qt::Dialog|Qt::CustomizeWindowHint);
    ...
    //ui->label_14->setText(("<font color=blue>"+QString("直流继电器(2.2.1)")+"</font>"));
...
{
    ui->label_14->setText(("<font color=blue>"+QString(tr("直流继电器(2.2.1)"))+"</font>"));
...
void Widget::vi_parameter_enter(int position)
{
    ...
    if(DCRelayType==1)
   {
    ...
    case 1:
        if(DCRelayType==1)
       { if(StartValue>MAX_V_VALUEDATA_DC*Maxfactor)
        StartValue=MAX_V_VALUEDATA_DC*Maxfactor;
        else if(StartValue<0)
        StartValue=0;
       }
       else
        {
            if(StartValue>MAX_I_VALUEDATA_DC*Maxfactor)
              StartValue=MAX_I_VALUEDATA_DC*Maxfactor;
            else if(StartValue<0)
              StartValue=0;
        }
        break;
    case 2:
        if(DCRelayType==1)
       { if(EndValue>MAX_V_VALUEDATA_DC*Maxfactor)
           EndValue=MAX_V_VALUEDATA_DC*Maxfactor;
          else if(EndValue<0)
           EndValue=0;
       }
       else
        {
            if(EndValue>MAX_I_VALUEDATA_DC*Maxfactor)
                EndValue=MAX_I_VALUEDATA_DC*Maxfactor;
              else if(EndValue<0)
                EndValue=0;
        }
          break;
    ...
    case 7:
        if(DCRelayType==1)
       { if(StopValue>MAX_V_VALUEDATA_DC*Maxfactor)
        StopValue=MAX_V_VALUEDATA_DC*Maxfactor;
        else if(StopValue<0)
        StopValue=0;
       }
       else
        {
            if(StopValue>MAX_I_VALUEDATA_DC*Maxfactor)
               StopValue=MAX_I_VALUEDATA_DC*Maxfactor;
              else if(StopValue<0)
            StopValue=0;
        }
...
void Widget::on_radioButton_clicked()
{
   ...
   OutputMode=1;
   if(DCRelayType==1)//dianya jidianqi
   {
       //起始值
       if(StartValue>MAX_V_VALUEDATA_DC)
       {
           StartValue=MAX_V_VALUEDATA_DC;
           ui->lineEdit->setText(QString::number(StartValue));
       }
       else if(StartValue<MIN_V_VALUEDATA_DC)
       {
           StartValue=MIN_V_VALUEDATA_DC;
           ui->lineEdit->setText(QString::number(StartValue));
       }
       //终止值
       if(EndValue>MAX_V_VALUEDATA_DC)
       {
           EndValue=MAX_V_VALUEDATA_DC;
           ui->lineEdit_2->setText(QString::number(EndValue));
       }
       else if(EndValue<MIN_V_VALUEDATA_DC)
       {
           EndValue=MIN_V_VALUEDATA_DC;
           ui->lineEdit_2->setText(QString::number(EndValue));
       }
   }
   else if(DCRelayType==2)//dianliu jidianqi
   {
       //起始值
       if(StartValue>MAX_I_VALUEDATA_DC)
       {
           StartValue=MAX_I_VALUEDATA_DC;
           ui->lineEdit->setText(QString::number(StartValue));
       }
       else if(StartValue<MIN_I_VALUEDATA_DC)
       {
           StartValue=MIN_I_VALUEDATA_DC;
           ui->lineEdit->setText(QString::number(StartValue));
       }
       //终止值
       if(EndValue>MAX_I_VALUEDATA_DC)
       {
           EndValue=MAX_I_VALUEDATA_DC;
           ui->lineEdit_2->setText(QString::number(EndValue));
       }
       else if(EndValue<MIN_I_VALUEDATA_DC)
       {
           EndValue=MIN_I_VALUEDATA_DC;
           ui->lineEdit_2->setText(QString::number(EndValue));
       }
   }
   switch(OutputMode)
...
void Widget::on_pushButton_2_clicked() //save
{
  if(save_f==0)
   ...
   if(current_language==1)
   {
               ...
               if(DCRelayType==1)
              { strcpy(strbuf,"直流电压继电器");
                fprintf(fp,"%s\r\n",strbuf);
              }
              else
               {
                 strcpy(strbuf,"直流电流继电器");
                 fprintf(fp,"%s\r\n",strbuf);
               }
              if(DCRelayType==1)
             {
   ...
   else if(current_language==2)
   {
       FILE *fp;
                      ...
                      if(DCRelayType==1)
                     { strcpy(strbuf,"Dc Voltage Relay test result");
                       fprintf(fp,"%s\r\n",strbuf);
                     }
                     else
                      {
                        strcpy(strbuf,"Dc Current Relay test result");
                        fprintf(fp,"%s\r\n",strbuf);
                      }
                     if(DCRelayType==1)
                    {
...
void Widget::on_pushButton_3_clicked() //闁偓閸
{
    if(starttest==1)
        return;
  uartthread->stoprun();
...
helpdialog->SetHelpfilename("Time.txt");
else if(current_language==1)
    helpdialog->SetHelpfilename("Time1.txt");
//helpdialog->SetHelpfilename("Time.txt");
helpdialog->show();
}

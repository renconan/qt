#include "voltageblock.h"
#include "ui_voltageblock.h"
#include <QTextCodec>
#include <QMouseEvent>
#include <QMessageBox>

VoltageBlock::VoltageBlock(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::VoltageBlock)
{
    ui->setupUi(this);
    QTextCodec::setCodecForTr(QTextCodec::codecForName("UTF-8"));
    QFont font("wenquanyi",16);
    setFont(font);
    setWindowFlags(Qt::Dialog|Qt::CustomizeWindowHint);
        setFixedSize(800,480);
            showFullScreen();
        ui->checkBox->setStyleSheet("QCheckBox{font:16px;}QCheckBox::indicator {width: 13px; height: 13px;}QCheckBox::indicator:checked {image: url(:/Check.png);}");
        ui->checkBox_2->setStyleSheet("QCheckBox{font:16px;}QCheckBox::indicator {width: 13px; height: 13px;}QCheckBox::indicator:checked {image: url(:/Check.png);}");

            ui->lineEdit_2->installEventFilter(this);
            ui->lineEdit_3->installEventFilter(this);
            ui->lineEdit_4->installEventFilter(this);
            ui->lineEdit_5->installEventFilter(this);
            ui->lineEdit_6->installEventFilter(this);
            ui->lineEdit_7->installEventFilter(this);
            ui->lineEdit_8->installEventFilter(this);
            ui->lineEdit_9->installEventFilter(this);
            ui->lineEdit_10->installEventFilter(this);
            ui->lineEdit_11->installEventFilter(this);
            ui->lineEdit_12->installEventFilter(this);

            ui->lineEdit->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_2->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_3->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_4->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_5->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_6->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_7->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_8->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_9->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_10->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_11->setContextMenuPolicy(Qt::NoContextMenu);
            ui->lineEdit_12->setContextMenuPolicy(Qt::NoContextMenu);

}

VoltageBlock::~VoltageBlock()
{
    delete ui;
}



bool VoltageBlock::eventFilter(QObject *obj, QEvent *event)
{


    if(event->type()==QEvent::MouseButtonPress)
    {

        return QWidget::eventFilter(obj,event);


    }
    else if(event->type()==QEvent::MouseButtonDblClick)
   {


        QMouseEvent *mouseevent=static_cast<QMouseEvent *>(event);
        if(mouseevent->button()==Qt::RightButton)
        {
            return true;
        }

        if(obj==ui->lineEdit_2)
        {
             currentedit=ui->lineEdit_2;

        }
        else if(obj==ui->lineEdit_3)
         {
              currentedit=ui->lineEdit_3;

         }
        else if(obj==ui->lineEdit_4)
         {
              currentedit=ui->lineEdit_4;

         }
        else if(obj==ui->lineEdit_5)
         {
              currentedit=ui->lineEdit_5;

         }
        else if(obj==ui->lineEdit_6)
         {
              currentedit=ui->lineEdit_6;

         }
        else if(obj==ui->lineEdit_7)
         {
              currentedit=ui->lineEdit_7;

         }
        else if(obj==ui->lineEdit_8)
         {
              currentedit=ui->lineEdit_8;

         }
        else if(obj==ui->lineEdit_9)
         {
              currentedit=ui->lineEdit_9;

         }
        else if(obj==ui->lineEdit_10)
         {
              currentedit=ui->lineEdit_10;

         }
        else if(obj==ui->lineEdit_11)
         {
              currentedit=ui->lineEdit_11;

         }
        else if(obj==ui->lineEdit_12)
         {
              currentedit=ui->lineEdit_12;

         }


        KeyPadDialog *pad=new KeyPadDialog;
        pad->SetText(currentedit->text());
        connect(pad,SIGNAL(SIG_sendtext(char*)),this,SLOT(setlineedittext(char *)));
        connect(pad,SIGNAL(SIG_closeflag()),this,SLOT(clearkeypadflag()));

        pad->setModal(true);
        pad->activateWindow();
        pad->raise();
        pad->show();


      return true;
   }
    else
    return QWidget::eventFilter(obj,event);

}


void VoltageBlock::setlineedittext(char *str)
{

    currentedit->setText(str);

}

void VoltageBlock::clearkeypadflag()
{

}



void  VoltageBlock::Setshowpara(struct SECTMENTSET value)
{

memcpy(&Sectvalue,&value,sizeof(struct SECTMENTSET));
ui->lineEdit->setText(tr("Voltage Blocking"));
ui->lineEdit->setEnabled(false);


ui->lineEdit_2->setText(Sectvalue.freset.TripVBlock.Dfdt.text);
ui->lineEdit_3->setText(Sectvalue.freset.TripVBlock.FreEndValue.text);
ui->lineEdit_4->setText(Sectvalue.freset.TripVBlock.VStart.text);
ui->lineEdit_5->setText(Sectvalue.freset.TripVBlock.VEnd.text);
ui->lineEdit_6->setText(Sectvalue.freset.TripVBlock.VStep.text);
ui->lineEdit_7->setText(Sectvalue.freset.TripVBlock.HoldTime.text);
ui->lineEdit_8->setText(Sectvalue.freset.Faultduraion.text);
ui->lineEdit_9->setText(Sectvalue.freset.TripVBlock.assessvblock.CompareID);

int VpnVpp=Sectvalue.freset.TripVBlock.VSelect;
ui->comboBox->setCurrentIndex(VpnVpp-1);




if(Sectvalue.freset.TripVBlock.assessvblock.relerror.valid)
{
 ui->checkBox->setChecked(true);
 ui->lineEdit_10->setText(QString::number(Sectvalue.freset.TripVBlock.assessvblock.relerror.errorvalue));

}
if(Sectvalue.freset.TripVBlock.assessvblock.abserror.valid)
{
 ui->lineEdit_11->setText(QString::number(Sectvalue.freset.TripVBlock.assessvblock.abserror.errorvalue,'g',5));
 ui->lineEdit_12->setText(QString::number(Sectvalue.freset.TripVBlock.assessvblock.abserror.errorvalue_Negtive,'g',5));

 ui->checkBox_2->setChecked(Sectvalue.freset.TripVBlock.assessvblock.abserror.valid);
}
 ui->comboBox_2->setCurrentIndex(Sectvalue.freset.TripVBlock.assessvblock.assessandor-1);

}

void  VoltageBlock::SetSettingLink(struct SETTINGNODELIST *link)
{
settinglink=link;
}

void VoltageBlock::copytoshuzu(char shuzu[], QString str, int len)
{
    if(len>=512)
        return;
    for(int i=0;i<len;i++)
        shuzu[i]=str.at(i).toAscii();
    shuzu[len]='\0';
}



void VoltageBlock::on_pushButton_clicked()
{


   if(ui->lineEdit_2->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input df/dt!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_3->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input end frequence!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_4->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input voltage start!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_5->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input voltage end!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_6->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input voltage step!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_7->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input step time!"),QMessageBox::Ok);
    return;
   }
   if(ui->lineEdit_8->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input tripping value keeping!"),QMessageBox::Ok);
    return;
   }
   if(ui->lineEdit_9->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input comparative id!"),QMessageBox::Ok);
    return;
   }


   if(CheckTextValid(settinglink,ui->lineEdit_2->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of df/dt is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_3->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of end frequence is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_4->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of voltage start is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_5->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of voltage end is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_6->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of voltage step is invalid!"),QMessageBox::Ok);
       return;
   }

   if(CheckTextValid(settinglink,ui->lineEdit_7->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of step time is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_8->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of tripping value keeping time is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_9->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of comparative id is invalid!"),QMessageBox::Ok);
       return;
   }


   if(ui->checkBox->isChecked())
   {
       if(ui->lineEdit_10->text().isEmpty())
       {
        QMessageBox::warning(this,tr("Warning"),tr("Please input relative error!"),QMessageBox::Ok);
        return;
       }

       if(CheckTextValid(settinglink,ui->lineEdit_10->text())!=0)
       {
           QMessageBox::warning(this,tr("Warning"),tr("Text of relative error is invalid!"),QMessageBox::Ok);
           return;
       }

   }

   if(ui->checkBox_2->isChecked())
   {
       if(ui->lineEdit_11->text().isEmpty())
       {
        QMessageBox::warning(this,tr("Warning"),tr("Please input positive absolute error!"),QMessageBox::Ok);
        return;
       }

       if(CheckTextValid(settinglink,ui->lineEdit_11->text())!=0)
       {
           QMessageBox::warning(this,tr("Warning"),tr("Text of positive absolute error is invalid!"),QMessageBox::Ok);
           return;
       }

       if(ui->lineEdit_12->text().isEmpty())
       {
        QMessageBox::warning(this,tr("Warning"),tr("Please input negative absolute error!"),QMessageBox::Ok);
        return;
       }

       if(CheckTextValid(settinglink,ui->lineEdit_12->text())!=0)
       {
           QMessageBox::warning(this,tr("Warning"),tr("Text of negative absolute error is invalid!"),QMessageBox::Ok);
           return;
       }

   }


   copytoshuzu(Sectvalue.freset.TripVBlock.Dfdt.text,\
               ui->lineEdit_2->text(),\
               ui->lineEdit_2->text().size());
   copytoshuzu(Sectvalue.freset.TripVBlock.FreEndValue.text,\
               ui->lineEdit_3->text(),\
               ui->lineEdit_3->text().size());
   copytoshuzu(Sectvalue.freset.TripVBlock.VStart.text,\
               ui->lineEdit_4->text(),\
               ui->lineEdit_4->text().size());
   copytoshuzu(Sectvalue.freset.TripVBlock.VEnd.text,\
               ui->lineEdit_5->text(),\
               ui->lineEdit_5->text().size());
   copytoshuzu(Sectvalue.freset.TripVBlock.VStep.text,\
               ui->lineEdit_6->text(),\
               ui->lineEdit_6->text().size());
   copytoshuzu(Sectvalue.freset.TripVBlock.HoldTime.text,\
               ui->lineEdit_7->text(),\
               ui->lineEdit_7->text().size());
   copytoshuzu(Sectvalue.freset.Faultduraion.text,\
               ui->lineEdit_8->text(),\
               ui->lineEdit_8->text().size());

   copytoshuzu(Sectvalue.freset.TripVBlock.assessvblock.CompareID,\
               ui->lineEdit_9->text(),\
               ui->lineEdit_9->text().size());
   Sectvalue.freset.TripVBlock.VSelect=ui->comboBox->currentIndex()+1;




   if(ui->checkBox->isChecked())
   {
                   QString value=ui->lineEdit_10->text();
                   bool ok;
                   Sectvalue.freset.TripVBlock.assessvblock.relerror.errorvalue=value.toFloat(&ok);
                   Sectvalue.freset.TripVBlock.assessvblock.relerror.valid=true;
   }
   else
   Sectvalue.freset.TripVBlock.assessvblock.relerror.valid=false;

   if(ui->checkBox_2->isChecked())
   {

               bool ok;
               QString value;
               value=ui->lineEdit_11->text();
               Sectvalue.freset.TripVBlock.assessvblock.abserror.errorvalue=value.toFloat(&ok);
               value.clear();
               value=ui->lineEdit_12->text();
               Sectvalue.freset.TripVBlock.assessvblock.abserror.errorvalue_Negtive=value.toFloat(&ok);
               Sectvalue.freset.TripVBlock.assessvblock.abserror.valid=true;

   }

   else
   Sectvalue.freset.TripVBlock.assessvblock.abserror.valid=false;

   Sectvalue.freset.TripVBlock.assessvblock.assessandor=ui->comboBox_2->currentIndex()+1;


 emit Sendpara((void *)&Sectvalue);
 emit sendflag();
 close();

}

void VoltageBlock::on_pushButton_2_clicked()
{
 emit sendflag();
 close();
}



#include "harmonicpara.h"
#include "ui_harmonicpara.h"
#include <QTextCodec>
#include <QMouseEvent>
#include <QMessageBox>

HarmonicPara::HarmonicPara(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::HarmonicPara)
{
    ui->setupUi(this);
...
HarmonicPara::~HarmonicPara()
{
    delete ui;
}


bool HarmonicPara::eventFilter(QObject *obj, QEvent *event)
{
...
void HarmonicPara::setlineedittext(char *str)
{
...
void HarmonicPara::clearkeypadflag()
{
...
void  HarmonicPara::Setshowpara(struct SECTMENTSET value)
{

memcpy(&Sectvalue,&value,sizeof(struct SECTMENTSET));
ui->lineEdit->setText(tr("Harmonic Test"));
ui->lineEdit->setEnabled(false);
ui->lineEdit_2->setText(Sectvalue.harmonicset.HarmonicCount.text);
ui->lineEdit_11->setText(Sectvalue.harmonicset.Current.text);
ui->lineEdit_12->setText(Sectvalue.harmonicset.Phase.text);

ui->checkBox_3->setChecked(Sectvalue.harmonicset.IAch);
ui->checkBox_4->setChecked(Sectvalue.harmonicset.IBch);
ui->checkBox_5->setChecked(Sectvalue.harmonicset.ICch);

ui->lineEdit_3->setText(Sectvalue.harmonicset.TripValue.StartValue.text);
ui->lineEdit_4->setText(Sectvalue.harmonicset.TripValue.EndValue.text);
ui->lineEdit_5->setText(Sectvalue.harmonicset.TripValue.StepValue.text);
ui->lineEdit_6->setText(Sectvalue.harmonicset.TripValue.HoldTime.text);


ui->lineEdit_7->setText(Sectvalue.harmonicset.TripValue.assesstripvalue.CompareID);

if(Sectvalue.harmonicset.TripValue.assesstripvalue.relerror.valid)
{
 ui->checkBox->setChecked(true);
 ui->lineEdit_8->setText(QString::number(Sectvalue.harmonicset.TripValue.assesstripvalue.relerror.errorvalue));

}
if(Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.valid)
{
 ui->lineEdit_9->setText(QString::number(Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.errorvalue,'g',5));
 ui->lineEdit_10->setText(QString::number(Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.errorvalue_Negtive,'g',5));

 ui->checkBox_2->setChecked(Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.valid);
 }
 ui->comboBox_2->setCurrentIndex(Sectvalue.harmonicset.TripValue.assesstripvalue.assessandor-1);

}

void  HarmonicPara::SetSettingLink(struct SETTINGNODELIST *link)
{
settinglink=link;
}

void HarmonicPara::copytoshuzu(char shuzu[], QString str, int len)
{
    if(len>=512)
        return;
    for(int i=0;i<len;i++)
...
void HarmonicPara::on_pushButton_clicked()
{
   if(ui->lineEdit_2->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input Harmonic number!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_3->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input Start harmonic!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_4->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input End harmonic!"),QMessageBox::Ok);
    return;
   }

   if(ui->lineEdit_5->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input Harmonic step!"),QMessageBox::Ok);
    return;
   }
   ...
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input Fundamental wave current!"),QMessageBox::Ok);
    return;
   }
   if(ui->lineEdit_12->text().isEmpty())
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input Fundamental wave phase!"),QMessageBox::Ok);
    return;
   }
   ...
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of Harmonic number is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_3->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of Start harmonic is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_4->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of End harmonic is invalid!"),QMessageBox::Ok);
       return;
   }
   if(CheckTextValid(settinglink,ui->lineEdit_5->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of Harmonic step is invalid!"),QMessageBox::Ok);
       return;
   }
   ...
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of Fundamental wave current is invalid!"),QMessageBox::Ok);
       return;
   }

   if(CheckTextValid(settinglink,ui->lineEdit_12->text())==-1)
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of Fundamental wave phase is invalid!"),QMessageBox::Ok);
       return;
   }
   ...
   copytoshuzu(Sectvalue.harmonicset.HarmonicCount.text,\
               ui->lineEdit_2->text(),\
               ui->lineEdit_2->text().size());
   copytoshuzu(Sectvalue.harmonicset.TripValue.StartValue.text,\
               ui->lineEdit_3->text(),\
               ui->lineEdit_3->text().size());
   copytoshuzu(Sectvalue.harmonicset.TripValue.EndValue.text,\
               ui->lineEdit_4->text(),\
               ui->lineEdit_4->text().size());
   copytoshuzu(Sectvalue.harmonicset.TripValue.StepValue.text,\
               ui->lineEdit_5->text(),\
               ui->lineEdit_5->text().size());
   copytoshuzu(Sectvalue.harmonicset.TripValue.HoldTime.text,\
               ui->lineEdit_6->text(),\
               ui->lineEdit_6->text().size());
   copytoshuzu(Sectvalue.harmonicset.TripValue.assesstripvalue.CompareID,\
               ui->lineEdit_7->text(),\
               ui->lineEdit_7->text().size());
   copytoshuzu(Sectvalue.harmonicset.Current.text,\
               ui->lineEdit_11->text(),\
               ui->lineEdit_11->text().size());
   copytoshuzu(Sectvalue.harmonicset.Phase.text,\
               ui->lineEdit_12->text(),\
   ...
   if(ui->checkBox->isChecked())
   {
       ...
       bool ok;
       Sectvalue.harmonicset.TripValue.assesstripvalue.relerror.errorvalue=value.toFloat(&ok);
       Sectvalue.harmonicset.TripValue.assesstripvalue.relerror.valid=true;
   }
   else
   Sectvalue.harmonicset.TripValue.assesstripvalue.relerror.valid=false;
   ...
   if(ui->checkBox_2->isChecked())
   {
               ...
               Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.errorvalue=value.toFloat(&ok);
               value.clear();
               value=ui->lineEdit_10->text();
               Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.errorvalue_Negtive=value.toFloat(&ok);
               Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.valid=true;

   }

   else
   Sectvalue.harmonicset.TripValue.assesstripvalue.abserror.valid=false;

   Sectvalue.harmonicset.TripValue.assesstripvalue.assessandor=ui->comboBox_2->currentIndex()+1;
...
void HarmonicPara::on_pushButton_2_clicked()
{
 emit sendflag();
 close();
}

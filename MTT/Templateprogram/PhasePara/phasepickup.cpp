#include "phasepickup.h"
#include "ui_phasepickup.h"
#include <QTextCodec>
#include <QMouseEvent>
#include <QMessageBox>

PhasePickup::PhasePickup(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::PhasePickup)
{
    ui->setupUi(this);
...
PhasePickup::~PhasePickup()
{
    delete ui;
}

bool PhasePickup::eventFilter(QObject *obj, QEvent *event)
{
...
void PhasePickup::setlineedittext(char *str)
{
...
void PhasePickup::clearkeypadflag()
{
...
void  PhasePickup::Setshowpara(struct SECTMENTSET value)
{
...
ui->lineEdit_4->setText(Sectvalue.phraseset.TripValue.StartValue.text);
ui->lineEdit_5->setText(Sectvalue.phraseset.TripValue.EndValue.text);
ui->lineEdit_6->setText(Sectvalue.phraseset.TripValue.StepValue.text);

ui->lineEdit_7->setText(Sectvalue.phraseset.TripValue.HoldTime.text);
ui->lineEdit_8->setText(Sectvalue.phraseset.Faultduraion.text);
ui->lineEdit_9->setText(Sectvalue.phraseset.TripValue.assesstripvalue.CompareID);

if(Sectvalue.phraseset.TripValue.assesstripvalue.relerror.valid)
{
 ui->checkBox_7->setChecked(true);
 ui->lineEdit_10->setText(QString::number(Sectvalue.phraseset.TripValue.assesstripvalue.relerror.errorvalue));

}
if(Sectvalue.phraseset.TripValue.assesstripvalue.abserror.valid)
{
 ui->lineEdit_11->setText(QString::number(Sectvalue.phraseset.TripValue.assesstripvalue.abserror.errorvalue,'g',5));
 ui->lineEdit_12->setText(QString::number(Sectvalue.phraseset.TripValue.assesstripvalue.abserror.errorvalue_Negtive,'g',5));

 ui->checkBox_8->setChecked(Sectvalue.phraseset.TripValue.assesstripvalue.abserror.valid);
 }

ui->comboBox->setCurrentIndex(Sectvalue.phraseset.TripValue.assesstripvalue.assessandor-1);

}

void  PhasePickup::SetSettingLink(struct SETTINGNODELIST *link)
{
settinglink=link;
}

void PhasePickup::copytoshuzu(char shuzu[], QString str, int len)
{
    if(len>=512)
        return;
    for(int i=0;i<len;i++)
...
void PhasePickup::on_pushButton_clicked()
{
   ...
   {
    QMessageBox::warning(this,tr("Warning"),tr("Please input start phase!"),QMessageBox::Ok);
    return;
   }
   ...
   {
       QMessageBox::warning(this,tr("Warning"),tr("Text of start phase is invalid!"),QMessageBox::Ok);
       return;
   }
   ...
   copytoshuzu(Sectvalue.phraseset.TripValue.StartValue.text,\
               ui->lineEdit_4->text(),\
               ui->lineEdit_4->text().size());
   copytoshuzu(Sectvalue.phraseset.TripValue.EndValue.text,\
               ui->lineEdit_5->text(),\
   ...
   copytoshuzu(Sectvalue.phraseset.TripValue.StepValue.text,\
               ui->lineEdit_6->text(),\
               ui->lineEdit_6->text().size());
   copytoshuzu(Sectvalue.phraseset.TripValue.HoldTime.text,\
               ui->lineEdit_7->text(),\
               ui->lineEdit_7->text().size());


   copytoshuzu(Sectvalue.phraseset.Faultduraion.text,\
               ui->lineEdit_8->text(),\
               ui->lineEdit_8->text().size());
   copytoshuzu(Sectvalue.phraseset.TripValue.assesstripvalue.CompareID,\
               ui->lineEdit_9->text(),\
   ...
   if(ui->checkBox_7->isChecked())
   {
                   ...
                   bool ok;
                   Sectvalue.phraseset.TripValue.assesstripvalue.relerror.errorvalue=value.toFloat(&ok);
                   Sectvalue.phraseset.TripValue.assesstripvalue.relerror.valid=true;
   }
   else
   Sectvalue.phraseset.TripValue.assesstripvalue.relerror.valid=false;
   ...
   if(ui->checkBox_8->isChecked())
   {
               ...
               Sectvalue.phraseset.TripValue.assesstripvalue.abserror.errorvalue=value.toFloat(&ok);
               value.clear();
               value=ui->lineEdit_12->text();
               Sectvalue.phraseset.TripValue.assesstripvalue.abserror.errorvalue_Negtive=value.toFloat(&ok);
               Sectvalue.phraseset.TripValue.assesstripvalue.abserror.valid=true;

   }

   else
   Sectvalue.phraseset.TripValue.assesstripvalue.abserror.valid=false;

   Sectvalue.phraseset.TripValue.assesstripvalue.assessandor=ui->comboBox->currentIndex()+1;
...
void PhasePickup::on_pushButton_2_clicked()
{
 emit sendflag();
 close();
}
